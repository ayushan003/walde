...
- Depth: 4 rows
- Width: 512 counters per row (per stripe)
- Total CMS memory: 64 stripes × 4 × 512 × 4 bytes = 512 KB
- Decay: halve all counters every 5×capacity stripe ops
- Decay trigger: separate `ops_since_decay_` counter, triggered
  only on put() calls (not get()) to keep the read path lean
- Increment policy: CMS is incremented on every get() HIT and
  every put(). Get misses do NOT increment CMS, which prevents
  scan keys from building false frequency on read-only workloads.
- Sizing rationale: each stripe holds ~128 items (8192/64).
  Width=512 gives a ~4:1 counter-to-item ratio per row, which
  at depth=4 keeps collision probability negligible.
