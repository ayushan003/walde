...
std::optional<std::string> CacheEngine::get(const std::string& key,
                                             LatencyBreakdown* bd) {
    // ── L1 lookup (stripe handles lock_wait + lookup timing) ─
    ...
    if (backend_result.has_value()) {
        if (bd) bd->path = LatencyBreakdown::Path::BACKEND_HIT;
        l1_.put(key, *backend_result);
        return backend_result;
    }

    if (bd) bd->path = LatencyBreakdown::Path::MISS;
    return std::nullopt;
}
...
size_t CacheEngine::get_l2_stripe_index(const std::string& key) const {
    return std::hash<std::string>{}(key) % kNumL2Stripes;
}
